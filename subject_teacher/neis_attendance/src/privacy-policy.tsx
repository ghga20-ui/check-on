...
/** 개인정보처리방침 모달. 사이드바 '개인정보처리방침'에서 진입.
 *  정본: docs/legal/privacy-policy.md */
export const PrivacyPolicy = ({ onClose }) => (
  <div className="ob-overlay" role="dialog" aria-modal="true" aria-label="개인정보처리방침">
    <div className="ob-card">
      <div className="ob-head">
        <div className="ob-head-text">
          <h2>개인정보처리방침</h2>
          <div className="ob-sub">체크온 · 시행일 2026-06-24 · 최종 개정 2026-07-13</div>
        </div>
      ...
      <div className="policy-body">
        ...
        <h3>5. NEIS 인증서 비밀번호</h3>
        <p>NEIS 출결 자동 입력 시 인증서 로그인을 자동으로 수행하기 위해 <b>NEIS 인증서 비밀번호</b>를
          입력받습니다. 비밀번호는 교사 PC의 <b>%LOCALAPPDATA%\NeisSubject</b> 폴더(password.bin)에
          Windows DPAPI로 암호화되어 <b>PC에만 저장</b>되며, 해당 Windows 사용자 계정에서만 복호화됩니다.
          클라우드(Google Drive)·서버 등 외부로는 절대 전송·저장되지 않으며, NEIS 로그인 자동화에만
          사용됩니다. 실행 화면의 비밀번호 입력란을 비우고 저장하면 저장된 값이 빈 값으로 대체(삭제)되고,
          위 폴더의 password.bin 파일을 직접 삭제해도 됩니다. 앱을 제거해도 이 파일은 자동 삭제되지
          않으므로, 완전히 파기하려면 위 폴더를 함께 삭제하세요.</p>
      ...
      <div className="ob-actions">
        <button className="tb-btn primary" onClick={onClose}>닫기</button>
      </div>
    </div>
  </div>
);
